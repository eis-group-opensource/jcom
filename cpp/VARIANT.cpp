...
//		var->vt = VT_EMPTY;
	...
	// VariantError なら VT_ERROR を設定
	jclass clsError = env->FindClass(CLASS_VARIANT_ERROR);
	if(env->IsInstanceOf(obj, clsError)) {
		jmethodID method = env->GetMethodID(clsError, "get", "()I");
		jint val = env->CallIntMethod(obj, method);
		var->vt = VT_ERROR;
		var->scode = val;
		return 0;	// 正常終了
...
int	VARIANT2jobject(JNIEnv* env, jobject obj, VARIANT* var, jobject* result)
{
	switch(var->vt) {
		...
		case VT_ERROR:	// VariantError を設定
		{
			jclass clsError = env->FindClass(CLASS_VARIANT_ERROR);
			jmethodID method = env->GetMethodID(clsError, "<init>", "(I)V");
			*result = env->NewObject(clsError, method, (jint)(var->scode));
			return 0;	// 正常終了
			break;
		}
